...
先构建渲染层产物到 `dist/renderer/`，再启动 Electron。
...
├── README.md
├── scripts/
│   └── copy-renderer-assets.js  # 拷贝 PNG 图标到 dist/renderer/
├── src/
│   ├── main/                     # Electron 主进程（TypeScript，tsc 构建 → dist/main/）
│   │   ├── main.ts               # 窗口（主面板/设置/悬浮球/QR）、托盘、IPC、Claude hooks 安装
│   │   ├── preload.ts            # contextBridge 安全预加载
│   │   ├── server.ts             # 内嵌 HTTP + WebSocket 服务器（端口 3456）
│   │   ├── detector.ts           # Claude Code 项目 jsonl 扫描器
│   │   ├── usage-monitor.ts      # Kimi / MiniMax / Copilot 用量轮询
│   │   ├── config.ts             # 配置持久化（userData/config.json）
│   │   └── pairing.ts            # 手机端配对二维码（host/port/apiKey）
│   ├── renderer/src/             # Vue 3 渲染层（Vite 构建 → dist/renderer/）
│   │   ├── main.ts / App.vue                 # 主面板
│   │   ├── settings.ts / Settings.vue        # 设置窗口
│   │   ├── floating-ball.ts / FloatingBall.vue  # 悬浮球
│   │   ├── components/           # TitleBar / ClaudeCard / UsageCard
│   │   ├── composables/          # useWebSocket / useUsageState
│   │   ├── utils/                # time / cwd（含 *.test.ts）
│   │   └── styles/
│   └── shared/                   # 主进程与渲染层共享
│       ├── constants.ts          # WS_PORT 唯一来源
│       ├── types/                # IPC / WebSocket / 用量等类型
│       └── utils/cwd.ts          # 跨进程 cwd 归一化
├── android-app/                  # 原生 Android 伴侣应用（Kotlin + Compose + Hilt）
